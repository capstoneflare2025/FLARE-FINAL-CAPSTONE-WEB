    ...
    <script>
    window.__STATION_KEY__ = "{{ session('station') }}";
    ...
    let fireSound, emergencySound, messageSound;
...
function showQueuedToast({ id, type, reporterName = 'Unknown', message = '', sound = null }) {
    if (!id || activeToasts.has(id)) return; // Skip if the toast already exists
    ...
    // Insert the new toast at the top of the toast container (prepend)
    toastContainer.prepend(toast);

    try { sound?.play?.().catch(() => {}); } catch {}

    // Store the toast element and set a timeout to remove it after 6 seconds
    const timeout = setTimeout(() => removeToast(id), 6000);  // Remove after 6 seconds
    activeToasts.set(id, { element: toast, timeout });

    // Optional: Adjust the stacking of toasts if there are more than 3
    if (toastContainer.children.length > 3) {
        // Fade out the oldest toast (the one at the bottom of the container)
        toastContainer.children[toastContainer.children.length - 1].classList.add('opacity-0');
        setTimeout(() => {
            toastContainer.children[toastContainer.children.length - 1].remove();  // Remove the oldest toast after fade
        }, 500);  // Wait for the fade out transition to complete
    }
}

// Function to remove a toast after it disappears
function removeToast(id) {
    ...
    setTimeout(() => {
        toastData.element.remove();  // Remove the toast element after fading out
    }, 500);

    clearTimeout(toastData.timeout);  // Clear the timeout if the toast is manually removed
    activeToasts.delete(id);  // Remove from active toasts map
}
    ...
    async function primeLastFFAdminKey(ref, storageKey) {
        ...
        console.warn('primeLastFFAdminKey failed', storageKey, e);
        }
    }


    async function attachFFAdminMessages(db) {
    let stationKey = window.__STATION_KEY__?.trim(); // Get the station key from session
    if (!stationKey) {
        console.warn("[FLARE] No station key defined, skipping AdminMessages listener.");
        return;
    }

    // Normalize: remove any accidental "CapstoneFlare/" prefix if present
    stationKey = stationKey.replace(/^CapstoneFlare\//, "");

    const refPath = `CapstoneFlare/${stationKey}/FireFighter/FireFighterAccount/AdminMessages`;
    const msgsRef = db.ref(refPath);
    const seenKey = `ff_last_adminmsg_${stationKey}`;

    console.log(`[FF AdminMessages] Listening on: ${refPath}`);

    // Prime last message so we don't trigger old messages
    await primeLastFFAdminKey(msgsRef, seenKey);

    // Listen for new messages
    msgsRef.orderByChild('timestamp').limitToLast(1).on('child_added', snap => {
        const id = snap.key;
        const m = snap.val() || {};
        console.log("Received AdminMessage:", m); // Log the full message object

        // Skip already-seen messages
        if (localStorage.getItem(seenKey) === id) return;
        localStorage.setItem(seenKey, id);

        // Only show messages from admin ("BFP")
        const sender = String(m.sender || "").trim().toLowerCase();
        console.log("Sender:", sender); // Log sender to verify it matches "BFP"
        if (sender !== "bfp") return;  // Only show messages from "BFP"

        console.log("🔥 FF AdminMessage Detected:", m);

        // Show notification with the preview of the admin message
        showQueuedToast({
        id: `ffchat|${stationKey}|${id}`,
        type: 'ffchat',
        reporterName: "BFP REPLY",
        message: ffPreviewFromMessage(m),
        sound: messageSound, // optional alert sound
        });
    });
    }
async function listenForMessageReplies() {
    ...
    document.addEventListener('DOMContentLoaded', async function () {
        ...
        // Attach listeners for AdminMessages
        attachFFAdminMessages(db);
          listenForMessageReplies();
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        messageSound = new Audio("{{ asset('sound/message_sound.wav') }}");
        messageSound.preload = "auto";
        fireSound.preload = emergencySound.preload = "auto";
        document.addEventListener('click', function unlockAudio() {
        try { fireSound.play().catch(() => {}); emergencySound.play().catch(() => {}); messageSound.play().catch(() => {}); } catch {}
        fireSound.pause(); emergencySound.pause(); messageSound.pause();
        fireSound.currentTime = 0; emergencySound.currentTime = 0; messageSound.currentTime = 0;
